...
export const SaleReturnModal: React.FC<Props> = ({ sale, isOpen, onClose, onSuccess }) => {
  ...
  useEffect(() => {
    ...
    setQuantities({});
    setReason('');
    salesService.getSaleById(sale.id, 'admin')
      .then((response) => setDetail(response.data || null))
      .catch((err) => setError(err instanceof Error ? err.message : 'Unable to load sale items.'))
  ...
  const submit = async () => {
    ...
    setError('');
    try {
      await salesService.returnSale(sale.id, { items: selected, refundMethod, reason });
      onSuccess();
      onClose();
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Unable to record the return.');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="fixed inset-0 z-[60] bg-slate-900/60 flex items-center justify-center p-4">
      <div className="w-full max-w-2xl rounded-2xl bg-white shadow-2xl overflow-hidden">
        <div className="flex items-center justify-between bg-slate-900 px-5 py-4 text-white">
          <div><h2 className="font-bold">Return sale items</h2><p className="text-xs text-slate-300">{sale.invoiceNumber}</p></div>
          <button onClick={onClose} className="p-2 rounded-lg hover:bg-white/10"><X className="w-5 h-5" /></button>
        </div>
        <div className="p-5 space-y-4 max-h-[70vh] overflow-y-auto">
          {error && <div className="flex gap-2 rounded-lg border border-rose-200 bg-rose-50 p-3 text-xs text-rose-700"><AlertCircle className="w-4 h-4" />{error}</div>}
            ...
            <div className="space-y-2">
              {(detail?.items || []).map((item) => (
                <div key={item.id} className="grid grid-cols-[1fr_auto] items-center gap-4 rounded-xl border border-slate-200 p-3">
                  <div><div className="text-sm font-semibold">{item.productName}</div><div className="text-xs text-slate-500">{item.companyName} · Sold {item.quantity} · {formatNaira(item.sellingPrice)}</div></div>
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            <select value={refundMethod} onChange={(e) => setRefundMethod(e.target.value as typeof refundMethod)} className="rounded-lg border border-slate-300 px-3 py-2 text-sm text-slate-900">
              <option value="CASH">Cash refund</option><option value="TRANSFER">Bank transfer</option><option value="POS">POS reversal</option>
            </select>
          ...
          <p className="text-xs text-amber-700">Outstanding debt is reduced first. Only the remaining collected amount is refunded.</p>
        </div>
        <div className="flex justify-end gap-3 border-t bg-slate-50 px-5 py-4">
          <button onClick={onClose} className="px-4 py-2 text-sm font-semibold">Close</button>
          <button onClick={submit} disabled={loading || submitting} className="inline-flex items-center gap-2 rounded-lg bg-rose-600 px-4 py-2 text-sm font-bold text-white disabled:opacity-50">
